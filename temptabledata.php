...
function getAll(){
    try{
        ...
        $query = "SELECT timedate, temp FROM weerstation ORDER BY timedate DESC LIMIT 200";
...
function getByDate($startdate, $enddate){
    try{
        require("secrets.php");
        $dbconnect= new PDO ("mysql:host=$servername;dbname=$dbname;charset=utf8","$username","$password");
        $query = "SELECT timedate, temp FROM weerstation WHERE timedate >= :start AND timedate <= :end ORDER BY timedate ASC";
        $stmt = $dbconnect->prepare($query);
